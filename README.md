...
```
- [function] is the real equation to be computed. These parameter must to follow the pattern [+ or -];[number];[x]^[number];[+ or -];[number];[x]^[number];[+ or -];[number];[x]^[number] (how many times desired). For instance, the input +;5;x^5;+;x^2;+;x^1 represents the equation 5x^5+x^2+x^1.
